...
from scheduler import start_orig_scheduler
...
async def post_application(application: schemas.CreateApplication,
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(f"{PE_URL}/agreement", json=application_item, headers=headers)
...
@app.get("/application/{agreement_id}", status_code=200, summary="Get applications by agreement",
         response_model=list[schemas.BaseApplication])
...
@app.post("/application/by/agreement", status_code=200, summary="Set new application",
